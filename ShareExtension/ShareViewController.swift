...
class ShareViewController: UIViewController {
    override func viewDidLoad() {
        ...
        let typeIdentifiers = [UTType.url.identifier, UTType.text.identifier, "public.file-url"]
        for identifier in typeIdentifiers {
            attachment.loadItem(forTypeIdentifier: identifier, options: nil) { (item, error) in
                DispatchQueue.main.async {
                    self.process(item: item, error: error, identifier: identifier)
                }
            }
        }
    }
